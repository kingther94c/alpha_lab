...
from quant_bot_manager.core.bot import get_bot


def _common(p):
    p.add_argument("--bot", default=config.DEFAULT_BOT)
    p.add_argument("--mode", choices=["demo", "testnet", "live"], default="demo")
    p.add_argument("--capital", type=float, default=config.DEFAULT_CONFIG["capital"])
    p.add_argument("--method", choices=["equal_capital", "risk_budget"], default="equal_capital")
    p.add_argument("--max-gross", type=float, default=config.DEFAULT_CONFIG["max_gross"])
...
def main():
    ...
    pr = sub.add_parser("run", help="continuous process"); _common(pr)
    pr.add_argument("--interval-min", type=float, default=config.DEFAULT_CONFIG["interval_min"])
    pr.add_argument("--max-cycles", type=int, default=0)
    prb = sub.add_parser("rebalance", help="one-shot rebalance"); _common(prb)
    prb.add_argument("--from-flat", action="store_true")
    ppl = sub.add_parser("plan", help="dry order plan (no auth)")
    ppl.add_argument("--bot", default=config.DEFAULT_BOT)
    ppl.add_argument("--capital", type=float, default=config.DEFAULT_CONFIG["capital"])
    ppl.add_argument("--method", choices=["equal_capital", "risk_budget"], default="equal_capital")
    args = ap.parse_args()
    config.load_env()
