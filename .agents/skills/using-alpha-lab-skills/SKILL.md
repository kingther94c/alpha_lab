...
description: >-
  ...
  research task, how the knowledge layers divide responsibility (AGENTS.md facts, AGENTS.md rules,
  ...
  deciding whether something belongs in a skill vs AGENTS.md vs a notebook cell, authoring or
...
| Layer | Owns | Loaded |
|---|---|---|
| `AGENTS.md` | Always-true **facts** + mechanical rules: interpreter path, "never touch `data/raw/`", cost/benchmark defaults, where code goes | Always |
| `AGENTS.md` | The agent **operating contract**: no-lookahead, trade-after-signal, extend-don't-proliferate, what "done" means | Always |
| `.Codex/skills/<name>/` | Reusable **procedures & techniques** that load only when relevant (ideate, backtest, audit, build a factor) | On trigger |
...
**De-dup rule:** a *fact* or *mechanical rule* lives in AGENTS.md/AGENTS.md; a *procedure* lives in a
...
- **Description** — third person, `"<what it does>. Use when <concrete triggers/keywords>."` Keyword-rich
  and slightly pushy (Codex under-triggers skills). ≤1024 chars.
...
- **Respect the layers** — a skill encodes a *procedure*. Facts and mechanical rules stay in
  AGENTS.md/AGENTS.md; ephemeral knobs stay in notebook cells.
